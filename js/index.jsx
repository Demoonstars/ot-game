/* =========================================================
   ФАЙЛ: js/index.jsx
   Точка входа: Сборка всех модулей и логика игры
========================================================= */

// 1. ИМПОРТЫ НАШИХ AAA-МОДУЛЕЙ
import { CAMPAIGNS, BOSS_CARDS, ACHIEVEMENTS_LIST, TERRITORY_UPGRADES } from './data.js';
import { AudioEngine, vibrate } from './audio.js';
import { ChatEngine } from './chatEngine.js';
import { CardEngine } from './CardEngine.jsx';
...
function Game() {
  ...
  const [messages, setMessages] = useState([
      { id: 1, from: "Руководство", text: "Шеф, завтра комиссия. Проверьте журналы инструктажей, штрафы платить не будем.", read: false }
  ]);
  ...
  const gameOverSequence = (victory = false) => {
      ...
      setTimeout(() => { 
          ...
          vibrate([500, 200, 500]); 
          setTimeout(() => { victory ? AudioEngine.winStamp() : AudioEngine.stamp(); }, 100); 
      }, 1200); 
  };
  ...
  const handleSwipe = (direction) => {
    ...
    // Обработка флагов (Троянские кони)
    const newFlags = { ...flags };
    if (currentCard.setFlag && direction === 'left') {
        newFlags[currentCard.setFlag] = true;
    }
    if (currentCard.id === 'm8' && direction === 'right') { // Взятка
        newFlags['bribe_given'] = true;
    }
    setFlags(newFlags);
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (newSafety <= 0 || newBudget <= 0 || newLoyalty <= 0) {
                ...
                if (currentCard.id === 'm8' && direction === 'right') unlockAchievement('bribe');
            ...
            // Вброс Босс-карт на основе флагов
      ...
      <div className="flex flex-col items-center justify-center h-[100dvh] px-6 text-center relative theme-morning">
        ...
        <div className="w-full max-w-xs flex flex-col gap-4 relative z-50">
            ...
            <div className="flex gap-4 mt-2">
                <button onClick={() => { AudioEngine.init(); setGameState('office')}} className="flex-1 bg-slate-800 text-slate-200 font-black py-3 rounded-full text-sm shadow-lg uppercase border border-slate-600 hover:bg-slate-700 active:scale-95 transition-all flex flex-col items-center justify-center gap-1">
                    <span>Ваш Кабинет</span><span className="text-[10px] text-yellow-500">{coins} 🪙</span>
                </button>
                <button onClick={() => { AudioEngine.init(); setGameState('achievements')}} className="flex-1 bg-slate-800 text-slate-200 font-black py-3 rounded-full text-sm shadow-lg uppercase border border-slate-600 hover:bg-slate-700 active:scale-95 transition-all flex flex-col items-center justify-center gap-1">
  ...
  /* --- РЕНДЕР: ИГРОВОЙ ЭКРАН (ПЛАНШЕТ СОТ) --- */
  return (
